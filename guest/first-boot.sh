...
cat > "$root/etc/pacman.d/ardesk.conf" <<EOF
RootDir = $root
...
EOF
if ! grep -q '^Include = /etc/pacman.d/ardesk.conf$' "$root/etc/pacman.conf"; then
    sed -i '/^\[options\]/a Include = /etc/pacman.d/ardesk.conf' "$root/etc/pacman.conf"
fi
...
cp "$root/usr/lib/ardesk-platform/ldconfig" "$root/usr/bin/ldconfig"
...
if [ ! -f "$root/etc/pacman.d/gnupg/ardesk-populated" ]; then
    echo 'ARDESK:正在初始化 Arch ARM 签名密钥…'
    pacman-key --init
